...
- (id)initWithTitle:(NSString *)title message:(NSString *)message timeout:(NSTimeInterval)timeout dismissible:(BOOL)dismissible
{
    ...
    if (self) {
        ...
        if (message) {
            ...
            _message.frame = CGRectMake(_message.frame.origin.x, 10, totalLabelWidth, messageSize.height);
...
- (void)show
{
    ...
    [UIView animateWithDuration:0.5 animations:^{
        self.alpha = 1;
        self.frame = CGRectMake(self.frame.origin.x, fullscreenRect.size.height - self.frame.size.height - bottomMargin + 8, self.frame.size.width, self.frame.size.height - 15);
